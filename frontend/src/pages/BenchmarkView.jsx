  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-emerald-50 border border-emerald-200/80 rounded-xl p-4 flex items-start gap-3 shadow-sm">
        ...
        <div>
          <h4 className="text-sm font-bold text-[#00543B]">Validation Limitation</h4>
          <p className="text-xs text-emerald-800 mt-1 font-semibold leading-relaxed">
            These numbers are an internal consistency check on synthetic data whose labels are generated by the same formulas
            used to build the training features — not proof of real-world predictive power. Run <code>python scripts/run_benchmark.py --n_customers 5000</code> to
            reproduce at scale, and see the README's Evaluation &amp; Validation section for how to retrain against real outcomes.
          </p>
        </div>
      </div>
        ...
        <>
          ...
          <Section title="Capacity Scoring Engine Evaluation" description={report.capacity_engine?.description}>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <MetricCard
                label="KS-Statistic"
                value={report.capacity_engine?.ks_statistic?.toFixed(4)}
                description="Credit-scorecard-standard separation between good/bad cumulative distributions."
              ...
              <MetricCard
                ...
                value={report.capacity_engine?.gini_coefficient?.toFixed(4)}
                description="2×AUC−1, the credit-scorecard-standard rescaling of AUC-ROC."
                icon={<BarChart3 className="w-5 h-5" />}
                colorClass="text-[#138B7B]"
                bgClass="bg-[#138B7B]/10"
              />
              <MetricCard
                label="PSI (Train vs. Test)"
                value={`${report.capacity_engine?.population_stability_index?.toFixed(4)} (${report.capacity_engine?.psi_interpretation || 'n/a'})`}
                description="Population Stability Index — the standard early-warning signal for score drift."
                icon={<Target className="w-5 h-5" />}
          ...
          <Section title="Risk Guardrail Engine Validation" description={report.guardrail_engine?.description}>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <MetricCard
                label="KS-Statistic"
                value={report.guardrail_engine?.ks_statistic?.toFixed(4)}
                description="Credit-scorecard-standard separation between stressed/safe cumulative distributions."
                icon={<Activity className="w-5 h-5" />}
                colorClass="text-[#00543B]"
                bgClass="bg-[#00543B]/10"
              />
              <MetricCard
                label="Gini Coefficient"
                value={report.guardrail_engine?.gini_coefficient?.toFixed(4)}
                description="2×AUC−1, the credit-scorecard-standard rescaling of AUC-ROC."
                icon={<BarChart3 className="w-5 h-5" />}
                colorClass="text-[#138B7B]"
                bgClass="bg-[#138B7B]/10"
              ...
              <MetricCard
                label="PSI (Train vs. Test)"
                value={`${report.guardrail_engine?.population_stability_index?.toFixed(4)} (${report.guardrail_engine?.psi_interpretation || 'n/a'})`}
                description="Population Stability Index — the standard early-warning signal for score drift."
                icon={<Target className="w-5 h-5" />}
                colorClass="text-[#F37021]"
                bgClass="bg-[#F37021]/10"
          ...
          <div className="bg-amber-50 border border-amber-200/60 rounded-xl p-5 text-center shadow-sm">
            <p className="text-xs text-amber-800 font-bold leading-relaxed">
              ⚠️ Validation limitation: these metrics are computed on held-out splits of synthetically generated data whose labels
              are produced by the same formulas used to build the training features — an internal consistency check, not proof of
              real-world predictive power. Before relying on this operationally, retrain against actual historical repayment/default
              outcomes (CapacityEngine.fit() / GuardrailEngine.fit() accept any dataset with the right target column).
            </p>
          </div>
        </>
      )}
